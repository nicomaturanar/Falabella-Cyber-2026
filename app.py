...
def get_all_items(orders, label="Cargando detalle de órdenes"):
    all_items = []
    total = len(orders)
    if total == 0:
        return pd.DataFrame()
...
def get_comparativo_anio_anterior(created_after, created_before):
    ...
    if created_before is None:
        ...
        fecha_fin_real = dt_module.date.fromisoformat(created_before[:10])
        hora_fin = hoy.strftime("%H:%M:%S") if fecha_fin_real >= hoy_date else "23:59:59"
...
with st.sidebar:
    st.header("⚙️ Período")
    modo = st.radio("", ["Hoy", "Últimas 24h", "Rango personalizado"], label_visibility="collapsed")
    if modo == "Rango personalizado":
        ...
        created_after  = f"{fecha_inicio}T00:00:00"
        created_before = f"{fecha_fin}T23:59:59"
    elif modo == "Últimas 24h":
        created_after  = (datetime.now(timezone.utc) - timedelta(hours=24)).strftime("%Y-%m-%dT%H:%M:%S+00:00")
        created_before = None
    else:
        medianoche_chile = ahora_chile.replace(hour=0, minute=0, second=0, microsecond=0)
        created_after  = medianoche_chile.strftime("%Y-%m-%dT00:00:00")
        created_before = None
...
# ── Año anterior ─────────────────────────────────────────────────────────────
ca_ant, cb_ant, fecha_ant = get_comparativo_anio_anterior(created_after, created_before)
with st.spinner(f"Cargando comparativo año anterior ({fecha_ant})..."):
...
df_orders_anterior = orders_to_df(orders_ant_raw) if orders_ant_raw else pd.DataFrame()
...
col1.metric("🛍️ Órdenes totales",  f"{total_ordenes:,}",
    delta=f"{var_pct(total_ordenes, total_ordenes_ant):+.1f}% vs {fecha_ant}" if total_ordenes_ant else None)
col2.metric("💰 Ventas (CLP)", clp(gmv_total),
    delta=f"{var_pct(gmv_total, gmv_ant):+.1f}% vs {fecha_ant}" if gmv_ant else None)
col3.metric("📦 Unidades vendidas", f"{total_items:,}",
    delta=f"{var_pct(total_items, total_items_ant):+.1f}% vs {fecha_ant}" if total_items_ant else None)
...
if not df_items_f.empty:
    ff_resumen = (df_items_f.groupby("fulfillment")
...
with tab1: st.bar_chart(hourly.set_index("hour_label")["ordenes"])
with tab2: st.bar_chart(hourly.set_index("hour_label")["gmv"])
...
if not df_items_f.empty:
    ...
    modelo_df = (df_items_f.groupby("modelo")
    ...
    mod_d = modelo_df.copy()
    mod_d["ventas"] = mod_d["ventas"].apply(clp)
    mod_d["share"]  = mod_d["share"].apply(lambda x: f"{x:.1f}%")
    mod_d.columns   = ["Modelo","Nombre","Marca","Línea","Categoría","Género","Órdenes","Unidades","Ventas (CLP)","Share %"]
    st.dataframe(mod_d, use_container_width=True, hide_index=True)
    st.divider()
    ...
    if not df_easy.empty:
        ...
        easy_d = easy_df.copy()
        easy_d["ventas"] = easy_d["ventas"].apply(clp)
        easy_d["share"]  = easy_d["share"].apply(lambda x: f"{x:.1f}%")
        easy_d.columns   = ["SKU 15","Nombre","Marca","Línea","Género","Unidades","Ventas (CLP)","Share %"]
        st.dataframe(easy_d, use_container_width=True, hide_index=True)
    else:
